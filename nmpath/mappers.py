    ...
    def r_mapper(coords):
        ...
        for bin_bounds_1D in bin_bounds:
            if any([bin_bounds_1D[i] >= bin_bounds_1D[i+1] for i in range(len(bin_bounds_1D)-1)]):
                raise Exception('For each dimension, the state bounds should increase monotonically')
        ...
        for j in range(n_variables):
            ...
            for k in range(n_partitions_in_dimension[j]):
                if bin_bounds[j][k] <= coords[j] < bin_bounds[j][k+1]:
                    ...
                    mapped = True; break
            if not mapped: 
                raise Exception('A coord value could not be mapped, make sure the values/bin_bounds are correct')
